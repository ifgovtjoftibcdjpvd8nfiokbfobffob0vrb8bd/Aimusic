...
def first_page_help_command(_):
    buttons = generate_help_buttons(_, start=1, end=15, current_page=1)
    navigation_buttons = [
        InlineKeyboardButton(text="ᴄʟᴏsᴇ", callback_data="close"),
        InlineKeyboardButton(text="ɴᴇxᴛ »", callback_data="help_next_help_2")
    ]
    buttons.append(navigation_buttons)
    return InlineKeyboardMarkup(buttons)
...
def second_page_help_command(_):
    buttons = generate_help_buttons(_, start=16, end=30, current_page=2)
    navigation_buttons = [
        InlineKeyboardButton(text="« ʙᴀᴄᴋ", callback_data="help_prev_help_1"),
        InlineKeyboardButton(text="ᴄʟᴏsᴇ", callback_data="close")
    ]
    buttons.append(navigation_buttons)
    return InlineKeyboardMarkup(buttons)
