        ...
        async def wrapper(client, message):
            if not message.from_user:
                await message.reply_text(
                    "You are an anonymous admin. Please unhide your account to use this command."
                )
                return
            ...
            if member.status == enums.ChatMemberStatus.OWNER:
                ...
                if missing_privileges:
                    await message.reply_text(
                        f"You don't have the required permissions: {', '.join(missing_privileges)}"
                    )
                    return
                return await func(client, message)
            else:
...
async def extract_user_and_title(message, client):
    ...
    if message.reply_to_message:
        ...
        if not args:
            await message.reply_text(
                "Please specify a user or reply to a user's message."
            )
            return None, None, None
        user_arg = args[0]
        try:
...
async def promote_command_handler(client, message):
    ...
    try:
        ...
        msg = format_promotion_message(
            chat_name, user_mention, admin_mention, action="promote"
        )
...
async def demote_command_handler(client, message):
    ...
    try:
        ...
        chat_name = message.chat.title
        msg = format_promotion_message(
            chat_name, user_mention, admin_mention, action="demote"
        )
        await message.reply_text(msg)
    except ChatAdminRequired:
        await message.reply_text("I need to be an admin with promote permissions.")
