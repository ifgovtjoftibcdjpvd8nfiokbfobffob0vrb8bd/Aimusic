            ...
            [
                [
                    InlineKeyboardButton(
                        "View Pack", url=f"https://t.me/addstickers/{pack_name}"
                    )
                ]
            ]
        ),
...
async def kang(client, message):
    ...
    if not reply or (
        ...
        return await processing_msg.edit(
            stylize_text("Reply to a sticker, photo, GIF, or video to kang it!")
        )
    ...
    try:
        if reply.sticker:
            ...
            if "image" in mime_type:
                ...
                await processing_msg.edit(
                    stylize_text("➣ Processing Animated Sticker...")
                )
                is_animated = True
                file_path = os.path.join(temp_dir, "kang_sticker.tgs")
                await reply.download(file_path)
        ...
        uploaded_document = await upload_sticker_file(
            client, user_id, media, is_animated, is_video
        )
        ...
        try:
            ...
            pack_title = get_pack_title(
                user_first_name, is_animated, is_video, pack_num
            )
            ...
            await processing_msg.edit(
                f"Flood wait error. Try again after {e.x} seconds."
            )
        except FileReferenceExpired:
            await processing_msg.edit(
                "The file reference has expired. Please resend the media and try again."
            )
...
async def upload_sticker_file(client, user_id, media, is_animated, is_video):
    ...
    attributes = [
        ...
        raw.types.DocumentAttributeSticker(
            alt="", stickerset=raw.types.InputStickerSetEmpty(), mask=False
        ),
    ]
