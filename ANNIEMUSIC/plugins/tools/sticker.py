...
async def generate_sticker(client, message):
    if len(message.command) == 2:
        ...
        await message.reply_text(
            stylize_text("Please provide a sticker ID after /st command.")
        )
...
async def download_sticker(client, message):
    ...
    if not message.reply_to_message or not message.reply_to_message.sticker:
        return await processing_msg.edit_text(
            stylize_text("Please reply to a sticker!")
        )
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not sticker.is_animated and not sticker.is_video:
                processing_msg = await processing_msg.edit_text(
                    stylize_text("➣ Processing Sticker...")
                )
                ...
                processing_msg = await processing_msg.edit_text(
                    stylize_text("➣ Sending Animated Sticker File...")
                )
                ...
                processing_msg = await processing_msg.edit_text(
                    stylize_text("➣ Sending Video Sticker...")
                )
        ...
        await processing_msg.edit_text(
            stylize_text(f"Error downloading sticker: {str(e)}")
        )
...
async def pack_kang(client, message):
    ...
    if not message.reply_to_message or not message.reply_to_message.sticker:
        return await processing_msg.edit_text(
            stylize_text("Reply to a sticker message")
        )
    ...
    try:
        ...
        for doc in sticker_set.documents:
            ...
            stickers.append(types.InputStickerSetItem(document=input_doc, emoji=emoji))
                ...
                [
                    [
                        InlineKeyboardButton(
                            "View Pack", url=f"https://t.me/addstickers/{short_name}"
                        )
                    ]
                ]
            ),
